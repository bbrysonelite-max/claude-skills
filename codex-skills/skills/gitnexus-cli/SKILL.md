...
**When to run:** First time in a project, after major code changes, or when `gitnexus://repo/{name}/context` reports the index is stale. In Codex, check staleness explicitly after `git commit` and `git merge` and run `analyze` when needed — the hook does not run analyze itself, to avoid blocking the agent for up to 120s and risking KuzuDB corruption on timeout.
...
- **Index is stale after re-analyzing**: Restart the active Codex session to reload the MCP server
- **Embeddings slow**: Omit `--embeddings` (it's off by default) or set `OPENAI_API_KEY` for faster API-based embedding

## Codex Runtime

Keep GitNexus as the external product name. Use actual GitNexus MCP tools and resources when available; otherwise preflight the CLI through `npx gitnexus`.
