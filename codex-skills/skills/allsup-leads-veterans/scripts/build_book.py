...
TEMPLATE = r"""<title>Allsup Veterans Leads — __DATE_ISO__</title>
<style>
  :root {
  ...
  *{box-sizing:border-box;}
  ...
  .src.reddit{background:var(--best-wash);color:var(--best);}.src.x{background:var(--good-wash);color:var(--ink);}.src.tiktok{background:var(--better-wash);color:var(--better);}
...
<div class="wrap">
  <header class="masthead">
    <p class="eyebrow">Allsup · Veterans benefits gap</p>
    <h1>Active veteran leads — __DATE_HUMAN__</h1>
    <p class="sub"><b>__TOTAL__ reachable veterans</b> pulled fresh from Reddit, X &amp; TikTok over the last 30 days, tiered by need. Each is a real veteran, in their own words, owed VA benefits they're not getting — with a public profile Allsup can reach. <b>Signal only — we never contact anyone.</b></p>
  </header>
  <div class="controls">
  ...
  <footer>
    Tiers — <b>Best:</b> denied / appealing / under-rated / never filed (owed but not getting it).
    <b>Better:</b> filing / gathering evidence / C&P exam / secondary condition.
    <b>Good:</b> eligibility / PACT Act / rating questions.<br>
    __TOTAL__ tiered veterans · reachability-graded, no scraping · generated __DATE_ISO__.
  </footer>
</div>
<script id="data" type="application/json">__DATA__</script>
<script>
  const TIERS={best:"Best",better:"Better",good:"Good"};
  const DESC={best:"denied · appealing · under-rated · never filed",better:"filing · evidence · C&P · secondary",good:"eligibility · PACT Act · rating questions"};
  ...
  function cardHTML(l){const full=l.q;const short=full.length>240?full.slice(0,240).replace(/\s+\S*$/,""):full;const truncated=full.length>short.length;const body=truncated?`${esc(short)}… <span class="more">(${full.length} chars — open source)</span>`:esc(full);const SRC={x:["x","X"],tiktok:["tiktok","TikTok"],reddit:["reddit","Reddit"]};const sc=(SRC[l.source]||SRC.reddit)[0];const sl=(SRC[l.source]||SRC.reddit)[1];return `<article class="card ${l.tier}"><blockquote>${body}</blockquote><div class="meta"><span class="handle">${esc(l.handle)}</span><span class="src ${sc}">${sl}</span><span class="links"><a class="link" href="${esc(l.source_post)}" target="_blank" rel="noopener">Post</a><a class="link primary" href="${esc(l.reachable_profile)}" target="_blank" rel="noopener">Profile</a></span></div></article>`;}
