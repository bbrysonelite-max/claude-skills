# graphify reference: extraction worker prompt

Load this in Step 3 Part B when the corpus has at least one doc, paper, or image chunk. A pure-code corpus skips Part B and never reads this file. Each semantic worker receives the prompt below verbatim (substitute FILE_LIST, CHUNK_NUM, TOTAL_CHUNKS, DEEP_MODE, and CHUNK_PATH).

```
You are a graphify extraction worker. Read the files listed and extract a knowledge graph fragment.
...
Then write the JSON to disk using filesystem editing at this exact absolute path (no relative paths — relative paths can resolve against the wrong working directory and the file will be silently lost):
CHUNK_PATH
```
