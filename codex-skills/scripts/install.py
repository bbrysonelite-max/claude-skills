...
def _previous_sources_preflight(
    previous_sources: tuple[Path, ...], current: _Source
) -> tuple[Path, ...]:
    roots: list[Path] = []
    seen: set[Path] = set()
    for candidate in previous_sources:
        ...
        if requested.is_symlink() or not requested.is_dir():
            raise ValueError(f"previous source is missing or unsafe: {requested}")
        root = requested.resolve(strict=True)
        canonical = root.parent.parent / "codex-skills" / "skills"
        if root != canonical or root.name != "skills":
            raise ValueError(
                "previous source must be a canonical codex-skills/skills directory: "
                f"{requested}"
            )
        if root == current.root:
            raise ValueError(f"previous source duplicates the current source: {root}")
        if root in seen:
            raise ValueError(f"duplicate previous source: {root}")

        marker = root / GENERATED_MARKER
        if marker.is_symlink() or not marker.is_file():
            raise ValueError(f"previous source is not builder-owned: {root}")
        try:
            marker_content = marker.read_bytes()
        except OSError as error:
            raise ValueError(
                f"previous source marker cannot be read: {marker}: {error}"
            ) from error
        if marker_content != GENERATED_MARKER_CONTENT:
            raise ValueError(f"previous source marker is invalid: {marker}")
        roots.append(root)
        seen.add(root)
    return tuple(roots)
...
def _destination_preflight(
    destination: Path, source: _Source, previous_sources: tuple[Path, ...] = ()
) -> Path:
    ...
    for previous_source in previous_sources:
        if _overlaps(resolved, previous_source):
            raise ValueError(f"destination overlaps previous source: {target}")
    return target
...
def _previous_managed_link_target(
    path: Path, name: str, previous_sources: tuple[Path, ...]
) -> tuple[Path, str] | None:
    try:
        raw_target = os.readlink(path)
    except OSError:
    ...
    if not candidate.is_absolute():
        candidate = path.parent / candidate
    target = candidate.resolve(strict=False)
    for root in previous_sources:
        if target == root / name and target.is_dir():
            return target, raw_target
    return None
...
def _plan_install(
    ...
    for name in source.names:
        ...
        if managed is None:
            managed = _previous_managed_link_target(path, name, previous_sources)
            if managed is None:
                collisions.append(name)
                continue
...
def install(
    ...
    *,
    ...
    try:
        ...
        approved_previous_sources = _previous_sources_preflight(
            tuple(Path(path) for path in previous_sources), source
        )
        target = _destination_preflight(
            Path(destination), source, approved_previous_sources
        )
        plan = _plan_install(
            source,
            target,
