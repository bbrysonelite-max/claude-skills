...
GENERATED_MARKER_CONTENT = b"Generated by codex-skills/scripts/build.py.\n"
...
def make_previous_source(root: Path, name: str = "previous") -> Path:
    source = root / name / "codex-skills" / "skills"
    source.mkdir(parents=True)
    (source / ".codex-skills-generated").write_bytes(GENERATED_MARKER_CONTENT)
    return source
...
class InstallTests(unittest.TestCase):
    ...
    def test_install_accepts_explicit_previous_sources(self):
        self.assertIn("previous_sources", inspect.signature(install).parameters)
    ...
    def test_migrates_only_same_name_links_from_an_approved_previous_source(self):
        previous = self.root / "old-repo" / "codex-skills" / "skills"
        shutil.copytree(COLLECTION, previous, symlinks=True)
        shutil.rmtree(previous / "graphify")
        old_names = tuple(
            sorted(path.name for path in previous.iterdir() if path.is_dir())
        )
        self.assertEqual(58, len(old_names))
        self.destination.mkdir(parents=True)
        for name in old_names:
            (self.destination / name).symlink_to(
                (previous / name).resolve(), target_is_directory=True
            )
        preserved_last30days = os.readlink(self.destination / "last30days")
        ...
        expected_updates = tuple(name for name in old_names if name != "last30days")
        self.assertTrue(dry.ok, dry.errors)
        self.assertEqual(("graphify",), dry.planned_created)
        self.assertEqual(expected_updates, dry.planned_updated)
        self.assertEqual(("last30days",), dry.excluded)
        self.assertEqual(
            (previous / "agent-reach").resolve(),
            (self.destination / "agent-reach").resolve(),
        )
        ...
        self.assertEqual(("graphify",), actual.created)
        self.assertEqual(expected_updates, actual.updated)
        self.assertEqual(("last30days",), actual.excluded)
        self.assertEqual(
            preserved_last30days, os.readlink(self.destination / "last30days")
        )
        ...
        self.assertEqual(58, len(rerun.unchanged))
        self.assertEqual((), rerun.created)
        self.assertEqual((), rerun.updated)
        self.assertEqual(("last30days",), rerun.excluded)

    def test_previous_source_does_not_authorize_a_wrong_name_link(self):
        previous = make_previous_source(self.root)
        for name in ("agent-reach", "ground-truth"):
            (previous / name).mkdir()
        self.destination.mkdir(parents=True)
        existing = self.destination / "agent-reach"
        existing.symlink_to(previous / "ground-truth", target_is_directory=True)
        old_target = os.readlink(existing)
        ...
        self.assertEqual(("agent-reach",), result.collisions)
        self.assertEqual(old_target, os.readlink(existing))
        self.assertEqual(
            [],
            [
                path
                for path in self.destination.iterdir()
                if path.name != "agent-reach"
            ],
        )

    def test_rejects_invalid_previous_sources_without_mutation(self):
        cases: list[tuple[str, Path, tuple[Path, ...]]] = []

        missing = self.root / "missing" / "codex-skills" / "skills"
        cases.append(("missing", missing, (missing,)))

        valid = make_previous_source(self.root, "symlink-target")
        linked = self.root / "linked-previous"
        linked.symlink_to(valid, target_is_directory=True)
        cases.append(("unsafe", linked, (linked,)))

        noncanonical = self.root / "noncanonical" / "skills"
        noncanonical.mkdir(parents=True)
        (noncanonical / ".codex-skills-generated").write_bytes(
            GENERATED_MARKER_CONTENT
        )
        cases.append(("canonical", noncanonical, (noncanonical,)))

        unmarked = self.root / "unmarked" / "codex-skills" / "skills"
        unmarked.mkdir(parents=True)
        cases.append(("builder-owned", unmarked, (unmarked,)))

        wrong_marker = make_previous_source(self.root, "wrong-marker")
        (wrong_marker / ".codex-skills-generated").write_bytes(b"not the marker\n")
        cases.append(("marker", wrong_marker, (wrong_marker,)))

        marker_symlink = make_previous_source(self.root, "marker-symlink")
        marker = marker_symlink / ".codex-skills-generated"
        marker.unlink()
        marker.symlink_to(self.root / "marker-target")
        (self.root / "marker-target").write_bytes(GENERATED_MARKER_CONTENT)
        cases.append(("builder-owned", marker_symlink, (marker_symlink,)))

        duplicate = make_previous_source(self.root, "duplicate")
        cases.append(("duplicate", duplicate, (duplicate, duplicate)))
        cases.append(("current", COLLECTION, (COLLECTION,)))

        for label, candidate, previous_sources in cases:
            with self.subTest(label=label, candidate=candidate):
                destination = self.root / f"destination-{label}"
                result = install(
                    COLLECTION,
                    destination,
                ...
                self.assertTrue(
                    any(label in error.casefold() for error in result.errors),
                    result.errors,
                )
                self.assertFalse(destination.exists())

    def test_destination_cannot_overlap_an_approved_previous_source(self):
        previous = make_previous_source(self.root)
        original_entries = tuple(previous.iterdir())

        for destination in (previous, previous / "installed", previous.parent.parent):
            with self.subTest(destination=destination):
                ...
                self.assertTrue(
                    any("overlaps previous source" in error for error in result.errors),
                    result.errors,
                )
                self.assertEqual(original_entries, tuple(previous.iterdir()))
...
class CliTests(unittest.TestCase):
    ...
    def test_cli_supports_repeatable_previous_sources(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            first = make_previous_source(root, "first")
            second = make_previous_source(root, "second")
    ...
    def test_cli_rejects_current_source_as_previous_source(self):
        with tempfile.TemporaryDirectory() as directory:
            destination = Path(directory) / "skills"
            stdout = io.StringIO()
            with redirect_stdout(stdout), redirect_stderr(io.StringIO()):
            ...
            self.assertEqual(1, exit_code)
            self.assertTrue(
                any("current" in error.casefold() for error in payload["errors"]),
                payload["errors"],
            )
            self.assertFalse(destination.exists())
